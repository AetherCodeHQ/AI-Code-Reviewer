...
> AI-Code-Reviewer - Open source tool by AetherCodeHQ

`ai` `cli` `code-review` `developer-tools` `golang` `open-source`

## What is it?

**Ai Code Reviewer** is a ai/ml tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./AI-Code-Reviewer --path ./target

# With options
./AI-Code-Reviewer --path ./target --format json --output report.json

# Verbose mode
./AI-Code-Reviewer --path ./target --verbose
```
...
### JSON report
```bash
./AI-Code-Reviewer --path ./my-project --format json --output report.json
```
...
- name: Run Ai Code Reviewer
  run: ./AI-Code-Reviewer --path . --format json --output report.json
```
